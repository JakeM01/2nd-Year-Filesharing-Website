...
public static function form_register_customer($countyTable,$pageID){
        ...
        //hidden DEFAULT input values for user type and login enabled
        $form.= '<input type="hidden" id="userTypeNr" name="userTypeNr" value="4">'; //default user type is customer
        $form.= '<input type="hidden" id="userEnabled" name="userEnabled" value="1">'; //default enabled to login is true
...
public static function form_upload_file($pageID){
    $form = '<form method="post" action="index.php?pageID=' . $pageID . '" enctype="multipart/form-data">';
    $form .= '<div class="form-group">';
    
    $form .= '<label for="fileToUpload">Select file to upload:</label>';
    $form .= '<input type="file" class="form-control" name="fileToUpload" accept=".jpg, .mp4, .mp3" required><br><br>';
    
    $form .= '<label for="file_permissions">Permissions:</label>';
    $form .= '<select class="form-control" name="file_permissions">';
    $form .= '<option value="Public">Public</option>';
    $form .= '<option value="Private">Private</option>';
    $form .= '</select><br><br>';
    
    $form .= '</div>';
    $form .= '<button type="submit" class="btn btn-default" name="submit">Upload File</button>';
    $form .= '</form>';
...
public static function form_search_File($pageID){
    $form='<form method="post" action="index.php?pageID='.$pageID.'">';
    $form.='<div class="form-group">';
    $form.='<label for="fileName">File Name (Enter  full File Name including extension e.g "filename.jpg")</label><input required type="text" class="form-control" id="fileName" name="fileName" >';
    $form.='</div> ';
    $form.='<button type="submit" class="btn btn-default" value="TRUE" name="btnSearch">Search</button>';
    $form.='</form>';
    return $form;
}   
public static function form_download_File($pageID, $fileName) {
    $form='<form method="post" action="index.php?pageID='.$pageID.'">';
    $form.='<div class="form-group">';
    $form .= '<input type="hidden" name="fileName" value="' . htmlspecialchars($fileName) . '">';
    $form.='</div> ';
    $form.='<button type="submit" class="btn btn-default" value="TRUE" name="btnDownload">Download</button>';
    $form .= '</form>';
    return $form;
}

public static function form_set_permissions($pageID) {
    $form = '<form method="post" action="index.php?pageID=' . $pageID . '" enctype="multipart/form-data">';
    $form .= '<div class="form-group">';
    $form.='<label for="file_name">File Name (Enter  full File Name including extension e.g "filename.jpg")</label><input required type="text" class="form-control" id="file_name" name="file_name" >';
    $form .= '<label for="file_permissions">Permissions:</label>';
    $form .= '<select class="form-control" name="file_permissions">';
    $form .= '<option value="Public">Public</option>';
    $form .= '<option value="Private">Private</option>';
    $form .= '</select><br><br>';
    $form .= '</div>';
    $form .= '<button type="submit" class="btn btn-default" name="btnSet">Set Permissions</button>';
    $form .= '</form>';
    return $form;

}

public static function form_delete_File($pageID){
    $form='<form method="post" action="index.php?pageID='.$pageID.'">';
    $form.='<div class="form-group">';
    $form.='<label for="file_name">File Name (Enter  full File Name including extension e.g "filename.jpg")</label><input required type="text" class="form-control" id="file_name" name="file_name" >';
    $form.='</div> ';
    $form.='<button type="submit" class="btn btn-default" value="TRUE" name="btnDelete">Delete</button>';
    $form.='</form>';
    return $form;
}  

public static function form_add_comment($pageID, $fileName)
{

    $form = '<form method="post" action="index.php?pageID=' . $pageID . '">';
    $form .= '<div class="form-group">';
    $form .= '<input type="hidden" name="file_name" value="' . htmlspecialchars($fileName) . '">';
    $form .= '<label for="comment_text">Enter your comment:</label>';
    $form .= '<textarea class="form-control" id="comment_text" name="comment_text" rows="5" required></textarea>';
    $form .= '<label for="rating">Rating (1-5):</label>';
    $form .= '<input type="number" class="form-control" id="rating" name="rating" min="1" max="5" required>';
    $form .= '</div>';
    $form .= '<button type="submit" class="btn btn-default" name="btnAddComment">Comment</button>';
    $form .= '</form>';
    return $form;
