...
class AdminDeleteFiles extends PanelModel{
    ...
    */  
    function __construct($user,$db,$postArray,$pageTitle,$pageHead,$pageID){  
        $this->modelType='AdminDeleteFiles';
    ...
    public function setPanelHead_1(){
            $this->panelHead_1='<h3>Delete Files</h3>';
    }
     /**
    ...
    public function setPanelContent_1(){
        ...
        $pageID=$this->pageID;
        $this->panelContent_1.= Form::form_delete_File($pageID);
        $filesTable = new FilesTable($this->db);
        if (isset($this->postArray['btnDelete'])) {
            $fileName = $this->postArray['file_name'];
            $this->panelContent_1 .= $filesTable->adminDeleteFiles($fileName);
        }
    }      
